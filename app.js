...
function render() {
  ...
  if (isDoctor() && !creatingConsult) {
    ...
    document.getElementById("btnAnalyses")?.addEventListener("click", () => {
      const consultId = lastSavedConsultId || (consultRows && consultRows.length ? consultRows[0].id : null);
      openAnalisesModal({ patientId: p.id, consultationId: consultId || null });
    });
  }
...
/* ==== INÍCIO BLOCO 12H — MÓDULO ANÁLISES LABORATORIAIS ==== */

/* ---- DADOS 12H.1 — Catálogo de grupos e análises ---- */
const ANALISES_CATALOG = [
  {
    id: "geral",
...
function openAnalisesModal({ patientId, consultationId }) {
  ...
  function buildModalHtml() {
    ...
    const groupsHtml = ANALISES_CATALOG.map(grp => {
      ...
      const itemsHtml = isOpen ? grp.items.map((item, idx) => {
        ...
        ">
          ...
          ">
            <div>
              ...
              <div style="font-size:12px;color:#64748b;margin-top:2px;">Selecione grupos · desselecione análises individuais se necessário</div>
            </div>
            <button id="gcAnalisesClose" style="
          ...
          <div style="padding:12px 20px;border-top:1px solid #e2e8f0;flex-shrink:0;">
            ...
            <textarea id="gcAnalisesCliInfo" placeholder="Ex: Suspeita espondiloartrite, dor lombar crónica…"
              style="width:100%;border:1px solid #cbd5e1;border-radius:8px;padding:8px 12px;font-size:13px;resize:vertical;min-height:60px;font-family:inherit;color:#0f172a;outline:none;"
          ...
          ">
            ...
            <button id="gcAnalisesGenPdf"
              ...
              style="padding:9px 22px;border:none;border-radius:8px;background:${hasSelected?"#1d9e75":"#cbd5e1"};color:${hasSelected?"#fff":"#94a3b8"};font-size:13px;font-weight:700;cursor:${hasSelected?"pointer":"not-allowed"};font-family:inherit;"
            >Gerar PDF</button>
          </div>
        </div>
      </div>`;
...
async function gerarAnalisePdf(state) {
...
function buildAnalisesHtml({ clinic, state, vinhetaUrl, logoUrl, signatureUrl }) {
  ...
  function renderColuna(items) {
    return items.map(e => e.tipo === "grupo"
...
/* ==== FIM BLOCO 12H — MÓDULO ANÁLISES LABORATORIAIS ==== */
