...
/* ==== INÍCIO BLOCO 06F/12 — Documentos/PDF (load + editor + gerar/upload) ==== */
  ...
  // ✅ FIX: NO TEU CASO o signedUrl para clinic-assets está a falhar (Object not found),
  // mas o download autenticado funciona. Então: usar APENAS download (mais estável).
  ...
  async function getClinicLogoDataUrl(clinic) {
    try {
      ...
      // Se for "bucket:path" (opcional), tenta storage direto (download autenticado)
...
`;
  ...
  async function htmlToPdfBlob(html, fileName) {
    ...
    // Host visível (Safari/Chrome mobile às vezes dá branco com offscreen)
  ...
  // ✅ FIX: garantir que, mesmo que o utilizador edite o HTML, as imagens ficam embutidas (dataURL)
  function forceEmbedImagesInHtml(html, vinhetaDataUrl, clinicLogoDataUrl) {
    let out = String(html || "");

    // Vinheta
    if (vinhetaDataUrl && vinhetaDataUrl.startsWith("data:")) {
      out = out.replace(
        /(<img\b[^>]*\bclass=["'][^"']*\bvinheta\b[^"']*["'][^>]*\bsrc=["'])([^"']*)(["'][^>]*>)/i,
        (m, a, _old, c) => `${a}${vinhetaDataUrl}${c}`
      );
      out = out.replace(/src=["']([^"']*dr-joao-morais-vinheta\.png)["']/gi, `src="${vinhetaDataUrl}"`);
      out = out.replace(/src=["']([^"']*vinheta\/dr-joao-morais-vinheta\.png)["']/gi, `src="${vinhetaDataUrl}"`);
    }

    // Logo
    if (clinicLogoDataUrl && clinicLogoDataUrl.startsWith("data:")) {
      out = out.replace(
        /(<img\b[^>]*\bclass=["'][^"']*\blogo\b[^"']*["'][^>]*\bsrc=["'])([^"']*)(["'][^>]*>)/i,
        (m, a, _old, c) => `${a}${clinicLogoDataUrl}${c}`
      );
    }
  ...
  async function generatePdfAndUploadV1() {
    try {
      ...
      // ✅ converter assets para dataURL (evita PDF branco e evita fetch a PNGs relativos)
      const vinhetaDataUrl = await getVinhetaDataUrl();
      const clinicLogoDataUrl = await getClinicLogoDataUrl(clinic);

      // Se estamos no editor visual/preview, sincroniza HTML atual
      if (docOpen && docMode !== "html") syncDocFromFrame();

      // Se não há draft, constrói base
      if (!docDraftHtml || docDraftHtml.trim().length < 300) {
        ...
        // ✅ respeita edição, mas força embed das imagens
        docDraftHtml = forceEmbedImagesInHtml(docDraftHtml, vinhetaDataUrl, clinicLogoDataUrl);
      }

      // garantia extra: se o HTML editado perdeu a vinheta, reconstrói base com assets
      if (vinhetaDataUrl && vinhetaDataUrl.startsWith("data:") && !/class=["'][^"']*\bvinheta\b/i.test(docDraftHtml)) {
        docDraftHtml = buildDocV1Html({ clinic, consult, authorName, vinhetaDataUrl, clinicLogoDataUrl });
      }
  ...
  // Export seguro (apenas 2 nomes, controlados)
  try {
