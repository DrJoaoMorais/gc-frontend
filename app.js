...
function mapAuthError(err) {
  ...
  if (/network|failed to fetch|fetch/i.test(msg)) {
    return "Sem ligação à internet (ou bloqueio de rede). Verifique a ligação e tente novamente.";
  }

  // Fallback com o texto técnico (mas não muito “cru”)
  return "Erro ao autenticar: " + msg;
}

function goApp() {
  // replace evita “voltar atrás” para o login com o botão Back
...
async function init() {
  try {
    ...
    // Não expor config no ecrã (mesmo que existam spans cfgUrl/cfgKey no HTML)
    if (el("cfgUrl")) el("cfgUrl").textContent = "";
    if (el("cfgKey")) el("cfgKey").textContent = "";

    // Se já houver sessão, vai direto para a app
    ...
    // Se fizer login noutro separador, este também redireciona
...
// Enter para submeter (evita múltiplos submits)
if (el("email"))
