import { clinicalHTML, editorHTML, loadClinicalHTML } from "./clinical-editor/content.js";
...
function openPatientViewModal(patient) {
  ...
  /* ================= SANITIZE/ESC ================= */
  function sanitizeHTML(html) { return clinicalHTML(html); }
...
/* ==== INÍCIO BLOCO 06G/12 — Timeline ==== */
  ...
  function renderHdaWithCollapsibleExams(hda) {
    ...
    if (sanitized.indexOf('EXAME OBJECTIVO') === -1 || /<(ul|ol)\b/i.test(sanitized)) {
      return sanitized;
    }
    ...
    document.addEventListener("click", async (ev) => {
      ...
      if (t && t.id === "btnSavePhysioRecord") {
        ...
        try {
          ...
          if (q && q.root) s.draftHtml = editorHTML(q) || "";
        } catch (_) {}
    ...
    const obs = new MutationObserver(() => {
      try {
        ...
        if (initialHtml.trim().length) {
          try { loadClinicalHTML(q, initialHtml); }
          catch (_) { q.setText(initialHtml); }
        } else {
          q.setText("");
        }

        enhanceClinicalEditor(q);
        q.on("text-change", () => {
          s.draftHtml = editorHTML(q) || "";
        });
    ...
    if (btnSave) {
      ...
      btnSave.onclick = async () => {
        ...
        try {
          ...
          if (q) draftHDAHtml = editorHTML(q) || "";
        } catch (_) {}
