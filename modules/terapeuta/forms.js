...
function fieldHtml(field, value, prefix) {
  ...
  if (field.type === 'choice' || (field.type === 'number' && (field.score || field.buttons))) {
    ...
    return `<fieldset class="field" data-field="${field.id}"><legend>${label}</legend><div class="choices">${values.map((v,i) => `<label><input type="radio" name="${key}" value="${i}" data-criterion="${escapeHtml(field.labels?.[v]||'')}" ${v===value?'checked':''}><span><b>${escapeHtml(v)}</b>${field.labels&&!field.compactLabels?` · ${escapeHtml(field.labels[v])}`:''}</span></label>`).join('')}</div>${field.labels&&field.compactLabels?`<p class="selected-criterion">${escapeHtml(field.labels[value]||'Escolhe a pontuação')}</p><details><summary>Ver critérios</summary>${values.map(v=>`<div>${escapeHtml(v)} — ${escapeHtml(field.labels[v])}</div>`).join('')}</details>`:''}</fieldset>`;
  }
