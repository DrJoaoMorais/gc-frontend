...
async function init(){
  try {
    if(preview){ const definitions=(await catalog()).filter(s=>preview.split(',').includes(s.id));if(!definitions.length)throw new Error('Escala desconhecida');context={definitions,instructions:'Pré-visualização. Podes experimentar; não serão enviados dados.'};index=0; }
      ...
      answers=context.answers||{};respondent=context.respondent||{};revision=context.revision;
    }
    render();
  }catch{message('Não foi possível abrir a avaliação. Verifica a ligação e volta a abrir o link.');}
}
function capture(){
 if(index<0){for(const key of ['name','profession','registration','date'])respondent[key]=root.querySelector('#meta_'+key).value;respondent.attested=root.querySelector('#attested').checked;return;}
...
function validateCurrent(){
 if(index<0){if(!respondent.name?.trim()||!respondent.profession?.trim()||!respondent.date||!respondent.attested)return 'Preenche a identificação, a data e confirma a declaração.';if(respondent.date>new Date().toISOString().slice(0,10))return 'A data da avaliação não pode ser futura.';return '';}
