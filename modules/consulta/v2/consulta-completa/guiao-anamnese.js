...
export function buildGuiaoHTML() {
  const linhasHTML = LINHAS.map(l =>
    `<div class="fc-guiao-linha"><span class="fc-guiao-k">${l.k}</span> — ${l.t}</div>`
  ).join('');
  ...
  return `
    <div class="fc-guiao-bar" id="fc-guiao-bar">
      ...
      <div class="fc-guiao-conteudo">
        ${linhasHTML}
        <div class="fc-guiao-linha fc-guiao-redflag">
          <i class="ti ti-alert-triangle" aria-hidden="true"></i> <span class="fc-guiao-k">Red flags?</span>
        </div>
      </div>
...
export function initGuiao(quill) {
  ...
  // arranca colapsado se já houver HDA escrita
  const temHDA = quill && quill.getText().trim().length > 0;
